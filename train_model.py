...
tf.app.flags.DEFINE_string(
    ...
    'The directory where the content dataset files are stored.')
tf.app.flags.DEFINE_string(
    'content_dataset_name', None,
    'The name of the content dataset to load.')
tf.app.flags.DEFINE_string(
    'content_dataset_split_name', 'train',
    'The name of the content train/test split.')

tf.app.flags.DEFINE_string(
    'style_dataset_dir', None,
    'The directory where the style dataset files are stored.')
tf.app.flags.DEFINE_string(
    'style_dataset_name', None,
    'The name of the style dataset to load.')
tf.app.flags.DEFINE_string(
    'style_dataset_split_name', 'train',
    'The name of the style train/test split.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        ######################
        # content dataset
        content_dataset = dataset_utils.get_split(
            FLAGS.content_dataset_name,
            FLAGS.content_dataset_split_name,
            FLAGS.content_dataset_dir)
        # style dataset
        style_dataset = dataset_utils.get_split(
            FLAGS.style_dataset_name,
            FLAGS.style_dataset_split_name,
            FLAGS.style_dataset_dir)

        ######################
        # create the network #
        ######################
        # parse the options from a yaml file
        style_model, options = models_factory.get_model(FLAGS.model_config)

        ###################################################
        # create dataset providers that load the datasets #
        ###################################################
        # 1) content dataset provider
        content_provider = slim.dataset_data_provider.DatasetDataProvider(
            content_dataset,
            num_readers=FLAGS.num_readers,
            common_queue_capacity=20 * FLAGS.batch_size,
            common_queue_min=10 * FLAGS.batch_size)
        [content_image] = content_provider.get(['image'])
        content_image_clip = preprocessing_image(
            content_image,
            style_model.default_size,
            style_model.default_size,
            style_model.content_size,
            is_training=True)
        content_image_clip_batch = tf.train.batch(
            [content_image_clip],
        ...
        style_provider = slim.dataset_data_provider.DatasetDataProvider(
            style_dataset,
            num_readers=FLAGS.num_readers,
            common_queue_capacity=20 * 1,
            common_queue_min=10 * 1)
        [style_image] = style_provider.get(['image'])
        style_image_clip = preprocessing_image(
            style_image,
            style_model.default_size,
            style_model.default_size,
            style_model.style_size,
            is_training=True)
        style_image_clip_batch = tf.train.batch(
            [style_image_clip],
            batch_size=1,
            num_threads=FLAGS.num_preprocessing_threads,
            capacity=32)

        # fetch queue the inputs
        batch_queue = slim.prefetch_queue.prefetch_queue(
            [content_image_clip_batch, style_image_clip_batch])

        ###########################################
        # build the model based on the given data #
        ###########################################
        contents, styles = batch_queue.dequeue()
        total_loss = style_model.build_model(contents, styles)
        ...
        learning_rate = _configure_learning_rate(
            content_dataset.num_samples, global_step)
